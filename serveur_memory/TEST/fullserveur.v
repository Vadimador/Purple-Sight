...
fn authentication(mut sock net.TcpConn,mut command_type &string) bool {
    // Shared secret key
    ...
    command_type = communication_type
    return true
}
...
fn handle_client(mut sock net.TcpConn) {
    ...
    if !authentication(mut sock,mut command_type) {
    ...
    if command_type == 'COMMAND' {
        ...
        for {
            ...
            cmd = os.input(" Your command : ")

            if !write_encrypt_message(cmd,key,iv,mut sock) {
                eprintln("erreur, lors de l'envoi de la commande")
            }
